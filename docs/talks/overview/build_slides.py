...
SOURCE = ROOT / "overview-slides.md"
OUTPUT = ROOT / "docs/_build/talks/overview/slides.qmd"
FRONT_MATTER = """---
        ...
        if match:
            ...
            if opened is None:
                ...
                if info.strip() == "mermaid":
                    if not convert:
                        raise ValueError(f"Plain Mermaid opening fence remains at line {number}")
                    line = (
                        indent + fence + info.replace("mermaid", "{mermaid}") + line[match.end() :]
                    )
                    if configure:
                        line += MERMAID_CONFIG
            elif fence[0] == opened[0][0] and len(fence) >= len(opened[0]) and not info.strip():
                opened = None
        lines.append(line)
...
def add_layout(markdown: str) -> str:
    ...
    for index, layout in layouts.items():
        if "Thin core, independent plugins" in lines[index]:
            layout = "diagram-wide"
...
def transform(markdown: str) -> str:
    ...
    body = body.replace(
        ...
        "PileatedWoodpeckerFeedingonTree%2C_crop.jpg/"
        "1280px-PileatedWoodpeckerFeedingonTree%2C_crop.jpg",
        "../../../talks/overview/assets/woodpecker.jpg",
    )
...
def build(source: Path = SOURCE, output: Path = OUTPUT) -> Path:
    ...
    for path, description in (
        ...
        (ROOT / "docs/talks/overview/assets/woodpecker.jpg", "Local Wikimedia photo"),
    ):
        if not path.is_file():
