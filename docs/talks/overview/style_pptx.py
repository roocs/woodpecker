"""Lay out Quarto's editable overview slides without a reference PPTX template.
...
Only the six-section main talk is supported; fail loudly when its structure changes.
"""
...
def style_text(shape, size=23, *, title=False):
    ...
    for body in shape.findall(".//p:txBody", NS) + shape.findall(".//a:txBody", NS):
        ...
        props.attrib.update(anchor="t", wrap="square", lIns="0", rIns="0", tIns="0", bIns="0")
        ...
        for paragraph in list(body.findall("a:p", NS)):
            ...
            ppr.attrib.update(
                marL=emu(0.23) if bullet else "0", indent=emu(-0.19) if bullet else "0", algn="l"
            )
            ...
            for rpr in runs:
                ...
                order_children(
                    rpr,
                    "a:ln a:noFill a:solidFill a:gradFill a:blipFill a:pattFill a:grpFill a:effectLst a:effectDag a:highlight a:uLnTx a:uLn a:uFillTx a:uFill a:latin a:ea a:cs a:sym a:hlinkClick a:hlinkMouseOver a:rtl a:extLst",
                )
            order_children(
                ppr,
...
def layout(shapes, index):
    ...
    expected = (1, 1, 1 if index == 4 else 0, 2)
    actual = (len(titles), len(pictures), len(tables), len(bodies))
    if actual != expected:
        raise ValueError(
            f"Main slide {index + 1}: expected title/image/table/body counts {expected}, got {actual}; update the PPTX layout"
        )
    title = titles[0]
    place(title, (0.65, 0.4, 12.05, 0.75))
    style_text(title, 36 if index == 0 else 30, title=True)
    if index == 0:
        boxes = [(0.65, 1.6, 7.35, 5.3), (8.55, 6.25, 4.1, 0.8)]
        diagram = (8.7, 1.65, 3.8, 4.4)
        sizes = [22, 12]
        ...
        boxes = [(0.65, 1.55, 7.0, 2.75), (0.65, 4.55, 12.05, 2.5)]
        diagram = (8.0, 1.55, 4.65, 2.75)
        sizes = [21, 21]
    elif index == 4:
        boxes = [(0.65, 2.15, 12.05, 2.95), (0.65, 6.0, 12.05, 1.15)]
        diagram = (1.25, 1.25, 10.8, 0.65)
        sizes = [17, 17]
        table = tables[0]
        place(table, (0.65, 5.1, 12.05, 0.75))
        style_text(table, 17)
        for col, width in zip(table.findall(".//a:gridCol", NS), [4.0, 8.05], strict=True):
            col.set("w", emu(width))
        for row_index, row in enumerate(table.findall(".//a:tr", NS)):
            row.set("h", emu(0.375))
            for cell in row.findall("a:tc", NS):
                fill = child(child(cell, "a:tcPr"), "a:solidFill")
                ET.SubElement(fill, tag("a:srgbClr"), val="DCEEF8" if row_index == 0 else "F3F4F6")
        for ppr in table.findall(".//a:pPr", NS):
            ppr.find("a:spcAft/a:spcPts", NS).set("val", "0")
    else:
        boxes = [(0.65, 1.4, 12.05, 0.6), (0.65, 4.6, 12.05, 2.5)]
        diagram = (0.9, 2.15, 11.55, 2.15)
        sizes = [24, 23]
        if index == 3:
            sizes[1] = 22
...
def style_pptx(source, output):
    ...
    if len(groups) != 6:
        raise ValueError(f"Expected six main sections, found {len(groups)}")
    removed = set()
    for index, group in enumerate(groups):
        ...
        rel_path = (
            posixpath.dirname(target_path) + "/_rels/" + posixpath.basename(target_path) + ".rels"
        )
    ...
    print(f"Styled six editable main slides: {output}")
